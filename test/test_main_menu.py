...
class MockWorksheet():
    ...
    def col_values(self, value):
        cols = [["empty col"], ["John"], ["john@gmail.com"], ["10"], ["4"],
                ["6"]]
        return cols[value]
...
class TestPlayer(unittest.TestCase):
    ...
    def test_update_database_value(self):
        self.assertEqual(self.player1.update_database_value(
            "name", "john", self.player1.email), [1, 1])
        self.assertEqual(self.player1.update_database_value(
            "email", "john", self.player1.email), [1, 2])
        self.assertEqual(self.player1.update_database_value(
            "total_games", "john", self.player1.email), [1, 3])
        self.assertEqual(self.player1.update_database_value(
            "wins", "john", self.player1.email), [1, 4])
        self.assertEqual(self.player1.update_database_value(
            "loses", "john", self.player1.email), [1, 5])

    @patch("main_menu.WORKSHEET", mock_worksheet)
    def test_add_player_to_database(self):
        self.assertEqual(
            self.player1.add_player_to_database(),
            ["John", "john@gmail.com", 10, 4, 6])

    @patch("main_menu.WORKSHEET", mock_worksheet)
    def test_check_is_email_registered(self):
        self.assertEqual(mm.check_is_email_registered(
            self.player1.email), True)
        self.assertEqual(mm.check_is_email_registered(
                "pat@gmail.com"), False)
    ...
    def test_regisiter_or_login_player(self):
        self.assertEqual(
            self.player1.register_or_login_player(),
            [self.player1.name, self.player1.email])
        self.assertEqual(
            self.player2.register_or_login_player(),
            [self.player2.name, self.player2.email,
             self.player2.total_games, self.player2.wins, self.player2.loses])

    def test_display_player_stats(self):
        self.assertEqual(
            self.player1.display_player_stats(),
            f"{Fore.CYAN + 'Name: ' + Fore.WHITE + 'John'}" +
            f"{Fore.CYAN + '   Email: ' + Fore.WHITE + 'john@gmail.com'}" +
            f"{Fore.CYAN + '   Total Games: ' + Fore.WHITE + '10'}" +
            f"{Fore.CYAN + '   Wins: ' + Fore.WHITE + '4'}" +
            f"{Fore.CYAN + '   Loses: ' + Fore.WHITE + '6'}")
...
class TestLogInPlayers(unittest.TestCase):
    ...
    def test_ask_player_email(self):
        self.assertEqual(
            mm.ask_player_email(self.player1.name), self.player1.email)
    ...
    def test_ask_incorrect_email_question1_1(self, mock_input):
        self.assertEqual(
            mm.ask_incorrect_email_question1(self.player1.name, True),
            self.player1.email)
    ...
    def test_ask_incorrect_email_question2_1(self, mock_input):
        self.assertEqual(
            mm.ask_incorrect_email_question2(
                self.player1.name, True, self.player1.email),
            self.player1.email)
    ...
    def test_incorrect_email_input(self):
        self.assertEqual(
            mm.incorrect_email_input(
                self.player1.name, True, self.player1.email),
            self.player1.email)
        self.assertEqual(
            mm.incorrect_email_input(
                self.player1.name, False, self.player1.email),
            self.player1.email)
    ...
    def test_validate_email_registered_1(self):
        self.assertEqual(
            mm.validate_email_registered(
                self.player1.email, True, self.player1.name),
            self.player1.email)
        self.assertEqual(
            mm.validate_email_registered(
                self.player1.email, False, self.player1.name),
            self.player1.email)
    ...
    def test_get_worksheet_value(self):
        self.assertEqual(
            mm.get_worksheet_value(self.player1.email, "total_games"), 10)
        self.assertEqual(mm.get_worksheet_value(self.player1.email, "wins"), 4)
        self.assertEqual(
            mm.get_worksheet_value(self.player1.email, "loses"), 6)
        self.assertEqual(
                mm.get_worksheet_value("pat@gmail.com", "total_games"), 0)
