...
def find_best_move(game_state, available_moves, ai_difficulty):
    ...
    if ai_difficulty == 1:
        ...
        find_move_nega_max_alpha_beta(game_state, available_moves,
                                      search_depth, -60, 60,
...
def find_move_nega_max_alpha_beta(game_state, available_moves, depth, alpha,
                                  beta, turn_multiplier, color):
    """
    ...
    for move in available_moves:
        ...
        next_moves = game_state.find_all_available_moves(
            get_opposite_color(color)
                )
        score = -find_move_nega_max_alpha_beta(game_state, next_moves,
                                               depth - 1, -beta, -alpha,
                                               -turn_multiplier,
                                               get_opposite_color(color))
        if score > max_score:
