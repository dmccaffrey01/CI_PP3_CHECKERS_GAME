...
"""
This file is responsible for storing all the information about the current
state of the chess game.
It will also be responsible for determining the valid moves at the
current state.
"""
...
class GameState():
    ...
    def find_all_available_moves(self, color):
        ...
        First the piece, second the moved position, third the option
        of the move, fourth the color
        """
        all_available_moves = []
    ...
    def find_available_moves(self, piece, color):
        ...
        available_moves = self \
            .format_available_moves(self
                                    .get_available_moves(piece_index, color))
    ...
    def get_available_moves(self, piece_index, color):
        ...
        if self.check_if_piece_on_edge_of_board(piece_index) == "Left":
            if self.check_if_piece_on_kings_edge(piece_index):
                if color == "white":
                    diaganol_right_king = self.get_diaganol(piece_index,
                                                            "Right-King",
                                                            color)
                    return ["blocked", "blocked", "blocked",
                            self.check_if_diaganol_empty(diaganol_right_king)]
                elif color == "black":
                    diaganol_right_normal = self.get_diaganol(piece_index,
                                                              "Right", color)
                    return ["blocked", self
                            .check_if_diaganol_empty(diaganol_right_normal),
                            "blocked", "blocked"]
            else:
                diaganol_right_normal = self.get_diaganol(piece_index,
                                                          "Right", color)
            ...
            if self.check_if_piece_on_kings_edge(piece_index):
                if color == "black":
                    diaganol_left_king = self.get_diaganol(piece_index,
                                                           "Left-King", color)
                    return ["blocked", "blocked",
                            self.check_if_diaganol_empty(diaganol_left_king),
                            "blocked"]
                elif color == "white":
                    diaganol_left_normal = self.get_diaganol(piece_index,
                                                             "Left", color)
                    return [self.check_if_diaganol_empty(diaganol_left_normal),
                ...
                if self.check_if_piece_is_kinged(piece_index, color):
                    ...
                    return [self.check_if_diaganol_empty(diaganol_left_normal),
                            "blocked", self
                            .check_if_diaganol_empty(diaganol_left_king),
                            "blocked"]
                else:
                    return [self.check_if_diaganol_empty(diaganol_left_normal),
                            "blocked"]
                ...
                if self.check_if_piece_on_kings_edge(piece_index) == "Top":
                    if color == "black":
                        ...
                        diaganol_left_king = self.get_diaganol(piece_index,
                                                               "Left-King",
                                                               color)
                        return ["blocked", "blocked", self
                                .check_if_diaganol_empty(diaganol_left_king),
                                self
                                .check_if_diaganol_empty(diaganol_right_king)]
                    elif color == "white":
                        diaganol_right_normal = self.get_diaganol(piece_index,
                                                                  "Right",
                                                                  color)
                        diaganol_left_normal = self.get_diaganol(piece_index,
                                                                 "Left", color)
                        return [self
                                .check_if_diaganol_empty(diaganol_left_normal),
                                self
                                .check_if_diaganol_empty(
                                    diaganol_right_normal
                                    ),
                                "blocked", "blocked"]
                elif self \
                        .check_if_piece_on_kings_edge(piece_index) == "Bottom":
                    if color == "white":
                        diaganol_right_king = self.get_diaganol(piece_index,
                                                                "Right-King",
                                                                color)
                        ...
                        diaganol_left_normal = self.get_diaganol(piece_index,
                                                                 "Left", color)
                        return [self.check_if_diaganol_empty(
                            diaganol_left_normal
                            ),
                            self.check_if_diaganol_empty(
                                diaganol_right_normal
                                ), "blocked", "blocked"]
    ...
    def check_kings_jumps(self, piece_index, left_or_right, color):
        ...
        if self.check_if_piece_is_kinged(self.original_piece_index, color):
            if self.check_if_piece_on_kings_edge(piece_index):
                ...
                elif self.check_if_piece_on_kings_edge(
                    piece_index
                        ) == "Bottom":
                    if color == "white":
                        self.get_jump(piece_index, direction_king, color)
            ...
            if (not self.check_if_piece_on_kings_edge(
                piece_index
                    ) == "Bottom" and color == "white") or \
                    (not self.check_if_piece_on_kings_edge(
                        piece_index) == "Top" and color == "black"):
                self.get_jump(piece_index, direction_normal, color)
    ...
    def get_jump(self, piece_index, left_or_right, color):
        ...
        if formatted_diaganol not in self.jumped_pieces_log:
            if self.check_if_piece_on_edge_of_board(diaganol) != \
                    left_or_right and not self.check_if_piece_on_kings_edge(
                        diaganol):
    ...
    def check_jump(self, diaganol, left_or_right, color):
        ...
        if self.check_if_diaganol_contains_opposite_color_piece(diaganol,
                                                                color):
            if not self.check_if_piece_on_edge_of_board(diaganol) and \
                    not self.check_if_piece_on_kings_edge(diaganol):
                new_diaganol = self.get_diaganol(diaganol, left_or_right,
                                                 color)
                if self.check_if_diaganol_empty(new_diaganol) != "blocked":
                    self.available_jumps.append(new_diaganol)
                    self.jump_count += 1
                    self.jumped_pieces_log.append(self.format_piece(
                        diaganol[0], diaganol[1]
                        ))
    ...
    def check_if_diaganol_contains_opposite_color_piece(self, diaganol, color):
        ...
        if (self.board[diaganol[0]][diaganol[1]] == "w" and color == "black") \
                or (self.board[diaganol[0]][diaganol[1]] == "W" and
                    color == "black"):
            return True
        elif (self.board[diaganol[0]][diaganol[1]] == "b" and
                color == "white") or \
                (self.board[diaganol[0]][diaganol[1]] == "B" and
                    color == "white"):
            return True
        else:
            return False
    ...
    def move_piece(self, piece, move, option, color):
        ...
        self.add_move_to_log(piece, move, option, removed_pieces, kinged,
                             color)
    ...
    def check_if_piece_needs_kinged(self, piece_index, color):
        ...
        """
        if (self.check_if_piece_on_kings_edge(
            piece_index
                ) == "Top" and color == "black") and \
                (not self.check_if_piece_is_kinged(piece_index, color)):
            self.king_piece(piece_index, color)
            return "Kinged"
        elif (self.check_if_piece_on_kings_edge(piece_index) == "Bottom" and
              color == "white") and (not self.check_if_piece_is_kinged(
                  piece_index, color
                  )):
            self.king_piece(piece_index, color)
            return "Kinged"
    ...
    def king_piece(self, piece_index, color):
        ...
        """
        if color == "black" and not self.check_if_piece_is_kinged(
            piece_index, color
                ):
            self.board[piece_index[0]][piece_index[1]] = "B"
        elif color == "white" and not self.check_if_piece_is_kinged(
            piece_index, color
                ):
    ...
    def add_move_to_log(self, piece, move, option, removed_pieces, kinged,
                        color):
        """
