...
def ask_user_to_sort_ranks():
    ...
    """
    print(Fore.YELLOW + "What would you like the leaderboard" +
          "to be sorted by?:")
    ...
    while True:
        ...
        if validated_option:
            ...
            break
        display.new_line()
        print(Fore.YELLOW + "Please input 1 or 2 or 3 for sort" +
              "or (4 to return):")
        option_selected = input(options)
...
def display_leaderboard_heading():
    ...
    heading = f"{top_bottom_of_leaderboard() + empty_leaderboard_line()}" + \
              f"{leaderboard_headings() + empty_leaderboard_line()}" + \
              f"{top_bottom_of_leaderboard()}"
    print(heading, end="")
    return heading
...
def empty_leaderboard_line():
    ...
    Returns f string
    """
    return f"{' ' * 16 + Fore.YELLOW + '|' + Fore.YELLOW + '|' + ' ' * 15}" + \
...
def leaderboard_headings():
    ...
    Returns f string
    """
    return f"{' ' * 16 + Fore.YELLOW + '|' + Fore.YELLOW + '|' + ' ' * 4}" + \
           f"{Fore.CYAN + 'R A N K' + ' ' * 4}" + \
           f"{Fore.YELLOW + '|' + ' ' * 4 + Fore.CYAN + 'N A M E'}" + \
           f"{' ' * 4 + Fore.YELLOW + '|' + ' ' * 4}" + \
           f"{Fore.CYAN + 'G A M E S' + ' ' * 4 + Fore.YELLOW + '|'}" + \
           f"{' ' * 4 + Fore.CYAN + 'W I N S' + ' ' * 4}" + \
           f"{Fore.YELLOW + '|' + ' ' * 4 + Fore.CYAN + 'L O S E S'}" + \
           f"{' ' * 4 + Fore.YELLOW + '|' + Fore.YELLOW + '|'}\n"
...
def display_leaderboard_ranks(sort_type):
    ...
    sorted_leaderboard_data = sort_leaderboard_data(leaderboard_data,
                                                    sort_type)
    for i, row in zip(range(1, len(sorted_leaderboard_data)+1),
                      sorted_leaderboard_data):
        print(f"{empty_leaderboard_line()}" +
              f"{leaderboard_data_line(row, i)}" +
              f"{empty_leaderboard_line() + top_bottom_of_leaderboard()}",
              end="")
        row.insert(0, i)
...
def sort_leaderboard_data(data, sort_type):
    ...
    sorted_data = sorted(data, key=itemgetter(sort_type), reverse=True) \
        if (sort_type == 2 or sort_type == 3) \
        else sorted(data, key=itemgetter(sort_type))
...
def leaderboard_data_line(row, i):
    ...
    return f"{' ' * 16 + Fore.YELLOW + '|' + Fore.YELLOW + '|' + rank}" + \
           ...
           f"{Fore.YELLOW + '|' + wins + Fore.YELLOW + '|' + loses}" + \
           f"{Fore.YELLOW + '|' + Fore.YELLOW + '|'}\n"
...
def format_leaderboard_rank_and_wins_and_name(str, type):
    ...
    if length == 1:
        ...
        return f"{Fore.WHITE + ' ' * 5 + str[0] + ' ' + str[1] + ' '}" + \
               f"{str[2] + ' ' * 5}"
    elif length == 4:
        return f"{Fore.WHITE + ' ' * 4 + str[0] + ' ' + str[1] + ' '}" + \
               f"{str[2] + ' ' + str[3] + ' ' * 4}"
    else:
        if type == "name":
            if length == 5:
                return f"{Fore.WHITE + ' ' * 3 + str[0] + ' '}" + \
                       f"{str[1] + ' ' + str[2] + ' ' + str[3] + ' '}" + \
                       f"{str[4] + ' ' * 3}"
            elif length == 6:
                return f"{Fore.WHITE + ' ' * 2 + str[0] + ' '}" + \
                       f"{str[1] + ' ' + str[2] + ' ' + str[3] + ' '}" + \
                       f"{str[4] + ' ' + str[5] + ' ' * 2}"
            else:
...
def format_leaderboard_games_and_loses(str):
    ...
    if length == 1:
        ...
        return f"{Fore.WHITE + ' ' * 6 + str[0] + ' ' + str[1] + ' '}" + \
               f"{str[2] + ' ' * 6}"
    elif length == 4:
        return f"{Fore.WHITE + ' ' * 5 + str[0] + ' ' + str[1] + ' '}" + \
               f"{str[2] + ' ' + str[3] + ' ' * 5}"
